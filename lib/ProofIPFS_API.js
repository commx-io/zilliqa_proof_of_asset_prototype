...
class ProofIPFS_API {
    ...
    async getRegistration(ipfs_cid) {
        ...
        if (this.chain_id != 111) {
            ...
            let state = await this.proof_ipfs.getSubState('ipfsInventory', [ipfs_cid]);
    ...
    async setPrice(new_price) {
        return this.proof_ipfs.call('setPrice',
            [{
                vname: 'new_price',
                type : 'Uint128',
    ...
    async registerOwnership(ipfs_cid, metadata) {
        ...
        .then(price => {
            ...
            return this.proof_ipfs.call('registerOwnership', params_call, params_register);
        })
        .then(callTx => {
